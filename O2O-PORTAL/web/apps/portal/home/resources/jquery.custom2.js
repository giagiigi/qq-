    ...
    */
                ...
                showSelected : function() {
                	...
                	toolbar.bind({
    	        	    click : function(event){
    	        	    	...
    	        	    	} else if("weixin-role-panel" == jqElm.attr("data-canvasrole")){
    	        	    		// 出现右侧编辑内容，图片列表
    	        	    		jq(this).imgList();
        	...
        	parent.iContent.find("a[name=canvasFile]").unbind().bind({
        	    click : function(){
        	    	if(!confirm('确定删除吗？')) {
        	    		...
        	    		if(1 == canvasControl.canvasUi.appPanel.iCount){
        	    			alert('至少有一条记录，不能删除。');
            	    		return;
            	    	}
            	    	canvasControl.canvasUi.appPanel.iCount--;
    ...
    jq.extend(CanvasWidget.AppPanel.prototype, {
    	init : function() {
            ...
            var addBtn = jq('<a href="#" style="margin-left:10px;"><i class="fa fa-plus-circle fa-lg"></i>&nbsp;增加</a>').unbind().bind( {
            	click : function() {
            		if(canvasControl.canvasUi.appPanel.iCount == canvasControl.canvasUi.appMax){
            			alert('已达到最大限制');
            			return;
            		}
            		parent.iCount++;
            		// 还需要向画布增加应用信息
            		var elemId = jq.getRandomDateBetween(new Date());
            		canvasControl.canvasUi.appPanel.iContent.append(jq('<div class="canvas-eachItem">' +
            				  '<div class="canvas-imgcontent-left"><img title="上传图片" id="'+canvasControl.canvasUi.curElm.attr("id")+'_'+jq.getRandomDateBetween(new Date()) +'" class="getImg" src="../resources/images/add.gif" width="100px" height="100px"/></div>' +
            				  '<div class="canvas-imgcontent-right">' +
            				    '<div style="height:45px;margin-left:5px;">' +
            				                       '跳转链接：<input type="text" maxlength="128" class="canvas-text-80 getTxt"/>' +
            				    '</div>' +
            				    '<div style="height:50px;margin-left:5px;">' +
            				                       '应用描述：<input type="text" maxlength="16" class="canvas-text-80 getTxt"/>' +
            				    '</div>' +
            				  '</div>' +
            				  '<div class="canvas-imgdel-link"><a id="'+ elemId + '"href="#" name="canvasFile"><i class="fa fa-trash-o fa-lg"></i></a></div>' +
            				'</div>'));
            		parent.addEvent();
            		parent.addAppElement(elemId);
            	}
            });
        ...
        addAppElement : function(elemId){
            // 增加应用消息
            var appItem = jq('<div style="position: relative;" id="image'+jq.getRandomDateBetween(new Date())+'"  class="canvas-event-selectable '

                 +'canvas-event-draggable ui-draggable ui-selectee ui-selected"  data-canvasrole="app-role-panel" '

                +'data-role="page"><img class="canvas_img_replace" alt="" src="../resources/images/add.gif" width="100%"> </div>');
            canvasControl.canvasUi.addEvent(appItem);

            if(undefined != canvasControl.canvasContent.find("div[data-canvasrole*='app-role-panel']").last() &&
           		 undefined != canvasControl.canvasContent.find("div[data-canvasrole*='app-role-panel']").last().html()){
           	 canvasControl.canvasContent.find("div[data-canvasrole*='app-role-panel']").last().after(appItem);
            } else {
           	 canvasControl.canvasContent.find("div").first().append(appItem);
            }
       },
       deleteAppElement : function(elemId){
           // 删除应用消息
           canvasControl.canvasContent.find(elemId).remove();
        },
        open : function(obj) {
        	this.iPanel.show();
            return this.iPanel;
        	...
        	parent.iContent.find("a[name=canvasFile]").unbind().bind({
        	    click : function(){
        	    	if(!confirm('确定删除吗？')) {
            	    	...
            	    	canvasControl.canvasUi.appPanel.iCount--;
            	    	var deleteElm = jq(this).parent().parent();
            	    	console.log(deleteElm);
            	    	// 删除画布对应的应用信息
                        var deleId = deleteElm.attr("id");
                        parent.deleteAppElement(deleId);
            	    	deleteElm.remove();
                        parent.refreshCanvasImg();
                        return false;
        	    	}
        	...
        	if('' != content && null != content && undefined != content){
        		...
        		for (var i in content) {
        			...
        			parent.iContent.append(jq('<div class="canvas-eachItem">' +
            				  ...
            				  '<div class="canvas-imgcontent-right">' +
          				    '<div style="height:45px;margin-left:5px;">' +
          				                       '跳转链接：<input type="text" maxlength="16" class="canvas-text-80 getTxt" value="'+(undefined == content[i][1] || "-1" == content[i][1] ? "":content[i][1])+'"/>' +
          				    '</div>' +
          				    '<div style="height:50px;margin-left:5px;">' +
          				                       '应用描述：<input type="text" maxlength="128" class="canvas-text-80 getTxt" value="'+(undefined == content[i][2] || "-1" == content[i][2] ? "":content[i][2])+'"/>' +
          				    '</div>' +
          				  '</div>' +
          				  '<div class="canvas-imgdel-link"><a id="'+ elemId + '"href="#" name="canvasFile"><i class="fa fa-trash-o fa-lg"></i></a></div>' +
          				'</div>'));
            	...
            	parent.iContent.append(jq('<div class="canvas-eachItem">' +
        				  ...
        				  '<div class="canvas-imgcontent-right">' +
        				    ...
        				    '</div>' +
        				  '</div>' +
        				  '<div class="canvas-imgdel-link"><a href="#" id="app100" name="canvasFile"><i class="fa fa-trash-o fa-lg"></i></a></div>' +
        				'</div>'));
            	parent.addEvent();
            }
        	parent.iButton.append(parent.iContent);
